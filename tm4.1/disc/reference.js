...
class Reference {
  ...
  concat(arr) {
    // @arr Ex: [ ['source','target','birthtime','comment'], ... ]
    // @arr Ex: [ {source,target,birthtime,comment}, ... ]
    if(Array.isArray(arr)) {
      arr.forEach(chunk=>{
        if(Array.isArray(chunk)) {
          let [source,target,birthtime,...comment] = chunk;
          comment=comment.join('\n');
          if(!Reference.checkByArray(source,target)){
            this.array.push({source,target,birthtime,comment});
          }

        }else if(chunk && chunk.source && chunk.target) {
          let {source,target,birthtime,...comment} = chunk;
          comment=Object.values(comment).join('\n');
          if(!Reference.checkByArray(source,target)){
            this.array.push({source,target,birthtime,comment});
          }
        }
    ...
    // this.array = Reference.uniqueCheckForArray(arr);
  }

  static checkByArray(source,target) {
    return this.arr.some((e,i,a)=>{
      return source===e[0] && target===a[i][1];
    });
  }
  ...
  search(s,p=0,i=0){
    ...
    this.array.forEach(function(e,index){
      var _s=e[i], sv=similarToNumber(s, e[i], true);
      if(sv >= p) {
...
function similarToNumber(a,b,c=true) {
  return Number(similar(a,b,c).toFixed(2));
}
