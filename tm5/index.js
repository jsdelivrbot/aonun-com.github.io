...
$(function () {
	...
	tm.getItem(formatName(location.search) + 'useGoogle', (j, v) => { if (j) { console.warn('[Error] No read useGoogle . ' + j.message); } else if (v) { $('#useGoogle').prop('checked', v); } });
	tm.getItem(formatName(location.search) + 'useNaver', (j, v) => { if (j) { console.warn('[Error] No read useNaver . ' + j.message); } else if (v) { $('#useNaver').prop('checked', v); } });
	tm.getItem(formatName(location.search) + 'useDaum', (j, v) => { if (j) { console.warn('[Error] No read useDaum . ' + j.message); } else if (v) { $('#useDaum').prop('checked', v); } });
	tm.getItem(formatName(location.search) + 'useDictTip', (j, v) => { if (j) { console.warn('[Error] No read useDictTip . ' + j.message); } else if (v) { $('#useDictTip').prop('checked', v); } });
	tm.getItem(formatName(location.search) + 'worksFontSize', (j, v) => { if (j) { console.warn('[Error] No read fontSize . ' + j.message); } else if (v) { $('#worksFontSize').val(v); changeWorksFontSize(); } });
...
$.ajax({
	...
	dataType: 'text',
	success(e,i,a){
		let u = URL.createObjectURL(new Blob([e],{type:'application/javascript'}));
		w = new Worker(u);
		w.onmessage=function(e){
			console.log(e.data);
		};
		w.postMessage('index.js -> log.js');
		w2 = new Worker(u);
		w2.onmessage = function (e) {
			console.log(e.data);
		};
		w2.postMessage({id:'w2',data:'文字'})
	}
});
