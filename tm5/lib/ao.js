		...
		var a=s.split('\n').forEach(function(e){
			...
			if(row.length===7){
				...
				if(row[5]){
					...
					$('<td />').text(row[1]).appendTo(tr).addClass('target').attr('contentEditable','plaintext-only');
					$('<td />').text(row[0]).appendTo(tr).addClass('textKey');
					$('<td />').text(row[6]).appendTo(tr).addClass('sourceComment');
					$('<td />').text(row[2]).appendTo(tr).addClass('targetComment').attr('contentEditable','plaintext-only');
				}else{
					$('<td />').text(row[1]).appendTo(tr).addClass('source');
					$('<td />').appendTo(tr).addClass('target').attr('contentEditable','plaintext-only');
					$('<td />').text(row[0]).appendTo(tr).addClass('textKey');
					$('<td />').appendTo(tr).addClass('sourceComment');
					$('<td />').appendTo(tr).addClass('targetComment').attr('contentEditable','plaintext-only');
				}
			}
		});
		return table.get(0);	
