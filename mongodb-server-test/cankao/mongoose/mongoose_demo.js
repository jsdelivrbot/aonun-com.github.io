...
var db       = mongoose.createConnection('mongodb://127.0.0.1:27017/mydb'); //数据库名：mydb
// 链接错误
...
mongooseSchema.methods.findbyusername = function(username, callback) {
    return this.model('mon').find({username: username}, callback);
}
// 添加 mongoose 静态方法，静态方法在Model层就能使用
mongooseSchema.statics.findbytitle = function(title, callback) {
    return this.model('mon').find({title: title}, callback);
}
// model
var mongooseModel = db.model('mon', mongooseSchema);//数据表名：mon (自动加+ s)

// 模块都是异步执行的，请单一测试。 Mongoose API : http://mongoosejs.com/docs/api.html
