importScripts('/lib/ao.similar.js');
console.log(ao)
var cn=/[\u4e00-\u9fff\uf900-\ufaff]/;
var ko=/[\u3131-\u314e\u314f-\u3163\uac00-\ud7a3]/;
	...
	if(typeof t==='string' && t.length>0 && dictArr instanceof Array && dictArr.length>0) {
		...
		dictArr.forEach(function(e,i){
			...
			for(var k in e){
				v=e[k];
				if(tisko){
					if(ko.test(v)) similar += ao.similar(t, v, true);
				}
				if(tiscn){
					if(cn.test(v)) similar += ao.similar(t, v, true);
				}
			}
			...
			// 	tmpArr.push([e, similar]);
			// }
			// similar=ao.similar(t,dict,true);
			b = similar>30;
			if(b) {
